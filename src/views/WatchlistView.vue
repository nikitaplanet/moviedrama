...
const sharedData = computed<ShareData | null>(() => (isReadonly.value ? decode(route.query.data as string) : null));

const activeFilters = computed<FilterState>(() =>
	isReadonly.value ? (sharedData.value?.filters ?? {category: '', country: '', status: ''}) : filters.value,
);

const sourceEntries = computed<Entry[]>(() => (isReadonly.value ? (sharedData.value?.entries ?? []) : entries.value));
...
<template>
	...
	<!-- Page header -->
	<AppPageHeader title="The Reel List" title-cn="片單" kicker="Personal Cinema · 私人放映室">
	...
	<!-- Draggable list -->
	<VueDraggable v-else-if="!isFiltered && !isReadonly" v-model="entries" :animation="200" class="flex flex-col" handle=".drag-handle">
		<EntryCard v-for="(entry, idx) in entries" :entry="entry" :key="entry.id" :rank="idx + 1" @remove="remove" @update="update" />
	</VueDraggable>
