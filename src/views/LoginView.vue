		...
		<div class="w-full max-w-sm">
			...
			<form class="card flex flex-col gap-4 p-6" @submit.prevent="submit">
				...
				<div class="flex flex-col gap-1">
					...
					<input v-model="password" class="field-input" autocomplete="current-password" placeholder="••••••••" required type="password" />
				</div>
				...
				<button :disabled="busy" class="btn-primary mt-1" type="submit">
			...
			<p class="mt-4 text-center text-sm" style="color: var(--muted)">
				<template v-if="mode === 'login'">
					還沒有帳號？
					<button class="underline" @click="mode = 'signup'" style="color: var(--accent)">立即註冊</button>
				</template>
				<template v-else>
					已有帳號？
					<button class="underline" @click="mode = 'login'" style="color: var(--accent)">返回登入</button>
				</template>
			</p>
		</div>
	</div>
